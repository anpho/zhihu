...
//    qDebug() << "page from cache?" << fromCache.toBool();
    if (reply->error() == QNetworkReply::NoError) {
        if (isARedirectedUrl(reply)) {
            ...
            const bb::ImageData imageDataVar = bb::ImageData::fromPixels(swappedImage.bits(), bb::PixelFormat::RGBX, swappedImage.width(), swappedImage.height(), swappedImage.bytesPerLine());
            setImage(Image(imageDataVar));;
        }
    }
    emit loadComplete();
    // Memory management
